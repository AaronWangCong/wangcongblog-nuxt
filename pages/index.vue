<template>
  <div class="index-wrap">
     <div id="container"> <canvas id="waterfall"></canvas> <div class="emma flex"> <div>  </div> </div> </div>
     <div class="route-box">
       <a class="nice-btn">进入次元</a>
       <nuxt-link to="/home" class="blog-btn">进入博客</nuxt-link>
     </div>
  </div>
</template>
<script>
  export default {
    ...
    mounted() {
      this.init(PIXI, window, document)
    },
    methods: {
      init(PIXI, window, document, undefined) {
  ...
  .route-box{
    ...
    a{
      ...
      &::after{
        ...
        border-top: 0;
        border-right: 0;
        background-image: linear-gradient(#ecd7c6,#ecd7c6);
      ...
      &.nice-btn{
        margin-right: 20px;
        background: rgba(221, 29, 29, 0.3);
        &:hover{
          background: rgba(221, 29, 29, 1);
        }
      }
      &.blog-btn{
        background: rgba(29, 74, 221, 0.3);
        &:hover{
          background: rgba(29, 74, 221, 1);
        }
      }
    }
  }
